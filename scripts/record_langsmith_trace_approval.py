"""Record human approval of the existing Step 5.1 trace without another trace write."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, default=DEFAULT_TRACE_APPROVAL_PATH)
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    approval = build_langsmith_trace_approval(
        expected_receipt_sha256=args.expected_receipt_sha256,
        reviewer=args.reviewer,
        reviewed_at=datetime.fromisoformat(args.reviewed_at),
    )
    _, digest = write_langsmith_trace_approval(approval, output_path=args.output)
